...
\begin{algorithm}[t]
\caption{FIGHI: Fisher-Information–Guided Hyperinteraction Inference}
\label{alg:fighi}
\begin{algorithmic}[1]
\Require
    CSV file with phenotype column $y$ and genotype columns $X_{1:p}$ (optionally an ID column); 
    trait type $\in\{\text{binary},\text{linear}\}$; maximum order $K_{\max}$; 
    target effect size proxy (e.g., odds ratio) $\text{OR}_0$; 
    prescreen parameters $(M,\, \texttt{row\_chunksize},\, \texttt{col\_block})$; \\
    pruning thresholds $(\epsilon_{\mathrm{var}},\, \epsilon_{\mathrm{maf}},\, \tau_{\mathrm{gain}})$;
    stability/permutation settings (optional).
\Ensure
    Hypergraph $H=(V,E)$ with edge weights $w_e=\Delta \mathcal{I}(e)$; 
    per-SNP FI summaries; reports and plots.

\Statex

\Function{FIGHI}{$\texttt{csv}, y\_\text{name}, \texttt{trait}, K_{\max}, \text{OR}_0, \texttt{prescreen}, \texttt{prune}$}
  \State // ---------- \textbf{Stage 0: Streaming Prescreen (optional)} ----------
  \State $S \gets$ \Call{Prescreen}{$\texttt{csv}, y\_\text{name}, M, \texttt{row\_chunksize}, \texttt{col\_block}$} \Comment{$|S|=M$ SNPs kept}
  \State Load dataframe $D \gets$ phenotype $y$ and columns $X_S$ (downcast to float32, mean-impute NaNs)
  \State Standardize (optional): z-score each $X_j$ and, for binary, leave $y\in\{0,1\}$; for linear, center $y$
  \State // ---------- \textbf{Stage 1: Initialize and plan $K$} ----------
  \State $K^* \gets$ \Call{PlannerMaxOrder}{$N,\, \mathrm{MAF}_{\min},\, \text{OR}_0,\, K_{\max}$} \Comment{analytical upper bound}
  \State $C_1 \gets$ \{ all singletons $\{j\}$ for $j\in S$\} \Comment{candidate sets of order $1$}
  \State $E \gets \emptyset$ \Comment{discovered edges}
  \State $\text{GainCum} \gets 0$; $\text{GainUpper} \gets$ theoretical max cum. gain for $K^*$ (optional)
  \State // ---------- \textbf{Stage 2: Adaptive growth across orders} ----------
  \For{$K \gets 1$ to $K^*$}
     \State // 2.1 score-test FI gain for each candidate at order $K$
     \For{each $e\in C_K$}
        \State $z_e \gets$ \Call{BuildFeature}{$e, X$}  \Comment{on-the-fly product $\prod_{j\in e} X_j$ (chunked if needed)}
        \If{ $\mathrm{Var}(z_e) < \epsilon_{\mathrm{var}}$ \textbf{ or } $\mathrm{MAF}(e) < \epsilon_{\mathrm{maf}}$ } \State \textbf{continue} \EndIf
        \State $(U_e, I_{ee}) \gets$ \Call{ScoreTestStats}{$z_e, X, y, \texttt{trait}$}
        \State $\Delta \mathcal{I}(e) \gets \tfrac{1}{2}\, U_e^2 / I_{ee}$ \Comment{Fisher Information gain}
        \If{$\Delta \mathcal{I}(e) \ge \tau_{\mathrm{gain}}$} 
           \State add $e$ to $E$ with weight $w_e=\Delta \mathcal{I}(e)$
        \EndIf
     \EndFor
     \State $\text{GainCum} \gets \text{GainCum} + \sum_{e\in E: |e|=K} w_e$
     \State // 2.2 stopping rule by information ratio (optional)
     \If{$\text{GainUpper}>0$}
        \State $r_K \gets \text{GainCum} / \text{GainUpper}$
        \If{$r_K > 0.95$} \textbf{break} \EndIf
     \EndIf
     \State // 2.3 Apriori expansion to order $K{+}1$
     \If{$K < K^*$}
        \State $C_{K+1} \gets$ \Call{ExpandCandidates}{$E, K$} \Comment{join-and-prune using frequent/high-gain subsets}
     \EndIf
  \EndFor
  \State // ---------- \textbf{Stage 3: Aggregation and export} ----------
  \State Per-SNP FI summaries: 
    $\mathrm{FI}_{\mathrm{main}}(s) \gets \sum_{e\in E, |e|=1, s\in e} w_e$; 
    $\mathrm{FI}_{\mathrm{interact}}(s) \gets \sum_{e\in E, |e|>1, s\in e} w_e$; 
    $\mathrm{FI}_{\mathrm{total}}(s) = \mathrm{FI}_{\mathrm{main}}(s)+\mathrm{FI}_{\mathrm{interact}}(s)$
  \State Construct hypergraph $H=(V=S,\, E)$ with order and weights; write GML/Cytoscape/JSON
  \State Save tables (interactions, per-SNP scores), model summary, logs, and plots
  \State \Return $H$, tables, artifacts
\EndFunction
\end{algorithmic}
\end{algorithm}
